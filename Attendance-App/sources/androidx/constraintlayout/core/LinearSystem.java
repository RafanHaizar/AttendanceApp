...
public class LinearSystem {
    ...
    public SolverVariable createObjectVariable(Object anchor) {
        ...
        if (anchor instanceof ConstraintAnchor) {
            ...
            if (variable.f979id == -1 || variable.f979id > this.mVariablesID || this.mCache.mIndexedVariables[variable.f979id] == null) {
                if (variable.f979id != -1) {
                    variable.reset();
                }
    ...
    private final int optimize(Row goal, boolean b) {
        ...
        while (!done) {
            ...
            if (goal.getKey() != null) {
                this.mAlreadyTestedCandidates[goal.getKey().f979id] = true;
            }
            SolverVariable pivotCandidate = goal.getPivotCandidate(this, this.mAlreadyTestedCandidates);
            if (pivotCandidate != null) {
                if (this.mAlreadyTestedCandidates[pivotCandidate.f979id]) {
                    return tries;
                }
                this.mAlreadyTestedCandidates[pivotCandidate.f979id] = true;
            }
            if (pivotCandidate != null) {
                            ...
                            if (a_j > f) {
                                ...
                                while (true) {
                                    ...
                                    if ((value < min && k == strength) || k > strength) {
                                        min = value;
                                        pivotRowIndex = i2;
                                        pivotColumnIndex = candidate.f979id;
                                        strength = k;
                                    }
                                    k++;
    ...
    private void displaySolverVariables() {
        System.out.println("Display Rows (" + this.mNumRows + SvgConstants.Attributes.f1641X + this.mNumColumns + ")\n");
    }
    ...
    public void addEquality(SolverVariable a, int value) {
        if (!USE_BASIC_SYNONYMS || a.definitionId != -1) {
            ...
            for (int i = 0; i < this.mVariablesID + 1; i++) {
                SolverVariable variable = this.mCache.mIndexedVariables[i];
                if (variable != null && variable.isSynonym && variable.synonym == a.f979id) {
