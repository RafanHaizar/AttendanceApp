...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f1020x;
        float position3 = position2;
        float position4 = motionPaths.f1021y;
        float endCenterX = motionPaths2.f1020x + (f / 2.0f);
        float endCenterY = motionPaths2.f1021y + (f3 / 2.0f);
        float pathVectorX = endCenterX - (f5 + (f2 / 2.0f));
        float pathVectorY = endCenterY - (position4 + (f4 / 2.0f));
        this.f1020x = (float) ((int) ((f5 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f1021y = (float) ((int) ((position4 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        ...
        this.mMode = 0;
        this.f1020x = (float) ((int) (((motionPaths.f1020x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f1021y = (float) ((int) (((motionPaths.f1021y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f)));
    ...
    public void setupRelative(MotionController mc, MotionPaths relative) {
        double dx = (double) (((this.f1020x + (this.width / 2.0f)) - relative.f1020x) - (relative.width / 2.0f));
        double dy = (double) (((this.f1021y + (this.height / 2.0f)) - relative.f1021y) - (relative.height / 2.0f));
        this.mRelativeToController = mc;
        this.f1020x = (float) Math.hypot(dy, dx);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f1021y = (float) (Math.atan2(dy, dx) + 1.5707963267948966d);
        } else {
            this.f1021y = (float) Math.toRadians((double) this.mRelativeAngle);
        }
    }
    ...
    public void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f1020x;
        float f6 = position2;
        float position3 = motionPaths.f1021y;
        float endCenterX = motionPaths2.f1020x + (f / 2.0f);
        float endCenterY = motionPaths2.f1021y + (f3 / 2.0f);
        this.f1020x = (float) ((int) ((f5 + ((endCenterX - (f5 + (f2 / 2.0f))) * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f1021y = (float) ((int) ((position3 + ((endCenterY - (position3 + (f4 / 2.0f))) * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        this.height = (float) ((int) (f4 + (scaleY * scaleHeight)));
        this.mMode = 2;
        if (!Float.isNaN(keyPosition.mPercentX)) {
            this.f1020x = (float) ((int) (keyPosition.mPercentX * ((float) ((int) (((float) parentWidth) - this.width)))));
        }
        if (!Float.isNaN(keyPosition.mPercentY)) {
            this.f1021y = (float) ((int) (keyPosition.mPercentY * ((float) ((int) (((float) parentHeight) - this.height)))));
        }
    ...
    public void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f = motionPaths.f1020x;
        float f2 = motionPaths.width;
        float f3 = motionPaths.f1021y;
        float f4 = position2;
        float position3 = motionPaths.height;
        float endCenterX = motionPaths2.f1020x + (motionPaths2.width / 2.0f);
        float endCenterY = motionPaths2.f1021y + (motionPaths2.height / 2.0f);
        float pathVectorX = endCenterX - ((f2 / 2.0f) + f);
        float pathVectorY = endCenterY - (f3 + (position3 / 2.0f));
        this.f1020x = (float) ((int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f1021y = (float) ((int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        ...
        float f6 = endCenterY;
        float f7 = (float) ((int) ((motionPaths3.f1020x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f1020x = f7;
        float f8 = pathVectorX;
        float f9 = (float) ((int) ((motionPaths3.f1021y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.f1021y = f9;
        this.f1020x = f7 + ((-pathVectorY) * perpendicular);
        this.f1021y = f9 + (pathVectorX * perpendicular);
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean diffx = diff(this.f1020x, points.f1020x);
        boolean diffy = diff(this.f1021y, points.f1021y);
        int c = 0 + 1;
    ...
    public void getCenter(double p, int[] toUse, double[] data, float[] point, int offset) {
        float f;
        int[] iArr = toUse;
        float v_x = this.f1020x;
        float v_y = this.f1021y;
    ...
    public void setView(float position2, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        int[] iArr = toUse;
        double[] dArr = slope;
        float v_x2 = this.f1020x;
        float v_y2 = this.f1021y;
        float v_width = this.width;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f1020x, this.f1021y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
