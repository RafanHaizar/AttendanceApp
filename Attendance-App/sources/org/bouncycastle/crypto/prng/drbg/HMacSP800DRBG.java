...
public class HMacSP800DRBG implements SP80090DRBG {
    ...
    private void hmac_DRBG_Update_Func(byte[] bArr, byte b) {
        this._hMac.init(new KeyParameter(this.f626_K));
        Mac mac = this._hMac;
        byte[] bArr2 = this.f627_V;
        mac.update(bArr2, 0, bArr2.length);
        this._hMac.update(b);
        if (bArr != null) {
            this._hMac.update(bArr, 0, bArr.length);
        }
        this._hMac.doFinal(this.f626_K, 0);
        this._hMac.init(new KeyParameter(this.f626_K));
        Mac mac2 = this._hMac;
        byte[] bArr3 = this.f627_V;
        mac2.update(bArr3, 0, bArr3.length);
        this._hMac.doFinal(this.f627_V, 0);
    }
