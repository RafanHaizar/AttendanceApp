...
class CommonFunction {
    ...
    public static short load16(byte[] bArr, int i) {
        ...
        if (bArr.length - i >= 2) {
            ...
            while (i2 < bArr.length - i) {
                s2 = (short) (s ^ (((short) (bArr[i + i2] & UByte.MAX_VALUE)) << (i2 * 8)));
                i2++;
            }
        }
        return s;
    ...
    public static int load32(byte[] bArr, int i) {
        ...
        if (bArr.length - i >= 4) {
            i2 = 0;
            while (i3 < 4) {
                i2 ^= (bArr[i + i3] & UByte.MAX_VALUE) << (i3 * 8);
                i3++;
            }
        } else {
            int i4 = 0;
            while (i3 < bArr.length - i) {
                i4 = i2 ^ ((bArr[i + i3] & UByte.MAX_VALUE) << (i3 * 8));
                i3++;
            }
        }
        return i2;
    ...
    public static long load64(byte[] bArr, int i) {
        ...
        if (bArr.length - i >= 8) {
            while (i2 < 8) {
                j ^= ((long) (bArr[i + i2] & UByte.MAX_VALUE)) << (i2 * 8);
                i2++;
            }
        } else {
            while (i2 < bArr.length - i) {
