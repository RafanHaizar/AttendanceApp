        ...
        try {
            ...
            if (subjectAlternativeNames != null) {
                for (List next : subjectAlternativeNames) {
                    ...
                    switch (intValue) {
                        case 2:
                            if (!isValidNameMatch(str, next.get(1).toString(), this.knownSuffixes)) {
                                break;
                            } else {
                                return true;
                            }
                        case 7:
                            if (!InetAddress.getByName(str).equals(InetAddress.getByName(next.get(1).toString()))) {
                                break;
                            } else {
                                return true;
                            }
                        default:
                            if (LOG.isLoggable(Level.INFO)) {
                                LOG.log(Level.INFO, "ignoring type " + intValue + " value = " + (next.get(1) instanceof byte[] ? Hex.toHexString((byte[]) next.get(1)) : next.get(1).toString()));
                                break;
                            } else {
                                break;
                            }
                ...
                for (int length = rDNs.length - 1; length >= 0; length--) {
                    ...
                    for (int i = 0; i != typesAndValues.length; i++) {
                        ...
                        if (attributeTypeAndValue.getType().equals((ASN1Primitive) BCStyle.f63CN)) {
